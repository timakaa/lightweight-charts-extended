// useLongPositionCursor.js - React hook for setting the mouse cursor style based on long position hover/drag state
import { useEffect } from "react";
import {
  getHoveredHandle,
  isPointOnLongPositionBody,
} from "../../drawing-tools/long-position/helpers";
...
// Sets the mouse cursor style for long position handles and body
function useLongPositionCursor(
  chart,
  candlestickSeries,
  hoveredPositionId,
  positionsDataRef,
      ...
      if (hoveredPositionId) {
        ...
        if (hovered) {
          ...
          if (handle) {
            ...
            isPointOnLongPositionBody(mouse, hovered, chart, candlestickSeries)
          ) {
            cursor = "pointer";
            setGlobalCursor(cursor);
            return;
      ...
      if (selectedPositionId) {
        ...
        if (selected) {
          ...
          if (handle) {
            ...
            isPointOnLongPositionBody(mouse, selected, chart, candlestickSeries)
          ) {
            cursor = "pointer";
            setGlobalCursor(cursor);
            return;
...
export default useLongPositionCursor;
