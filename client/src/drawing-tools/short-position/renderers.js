...
export function drawLabelShort(
  ...
  // You can customize this if you want a different style for short
  drawLabel(ctx, x, y, textLines, color, above, offset);
}

// ShortPositionPaneRenderer draws the profit/loss boxes, entry line, and info panels for the short position
export class ShortPositionPaneRenderer {
  ...
  // Draws the short position (profit/loss zones, entry line, and info panels)
  draw(target) {
    target.useBitmapCoordinateSpace((scope) => {
      ...
      if (
        ...
        if (paneView && candleData && Array.isArray(candleData)) {
          ...
          if (entryIdx !== -1 || targetIdx !== -1) {
            ...
            if (tapIdx !== -1) {
              // 2. After tap, check for stop/target hit (short logic) within available data
              let endX = null;
              let endY = null;
              let hit = false;
              ...
              for (let i = tapIdx; i <= searchEnd; ++i) {
                ...
                // Stop loss hit (short: price >= stop)
                if (c.high >= stopPrice) {
                  endX = x;
                  endY =
                ...
                // Take profit hit (short: price <= target)
              ...
              if (endX !== null && endY !== null) {
                ...
                // --- HIGHLIGHT AREA PASSED BY ARROW (short logic) ---
                if (paneView._profitBox && paneView._lossBox) {
                  if (endY > startY) {
                    // Arrow ends in profit box (short)
                    // More opaque profit color
                    let rgba = this._options.fillColor
                      .replace(/rgba?\(([^)]+)\)/, "$1")
                      .split(",");
                    if (rgba.length === 3) rgba.push("0.41");
                    else
                    ...
                    // Arrow ends in loss box (short)
                    // More opaque loss color
                    let rgba = this._options.lossFillColor
                      .replace(/rgba?\(([^)]+)\)/, "$1")
                      .split(",");
                    if (rgba.length === 3) rgba.push("0.24");
                    else
                      rgba[3] = Math.min(1, parseFloat(rgba[3]) + 0.1).toFixed(
                        2,
                      );
      ...
      if (showPanels && paneView && paneView._profitBox && paneView._lossBox) {
        ...
        // Calculate percent changes (INVERT for short)
        ...
        let stopPercent = "";
        if (entry && target) {
          // For short: profit percent is always positive, always '+'
          const pct = ((entry - target) / entry) * 100;
          targetPercent = `+${Math.abs(pct).toFixed(2)}%`;
        }
        if (entry && stop) {
          // For short: loss percent is always negative, always '-'
          const pct = ((entry - stop) / entry) * 100;
          stopPercent = `-${Math.abs(pct).toFixed(2)}%`;
        }
        // Profit panel (centered, directly below profit box)
        let profitCenterX =
          (paneView._profitBox.x + paneView._profitBox.width / 2) *
          scope.horizontalPixelRatio;
        const profitY =
          (paneView._profitBox.y + paneView._profitBox.height) *
          scope.verticalPixelRatio; // bottom edge in px
        // Loss panel (centered, directly above loss box)
        let lossCenterX =
          (paneView._lossBox.x + paneView._lossBox.width / 2) *
          scope.horizontalPixelRatio;
        const lossY = paneView._lossBox.y * scope.verticalPixelRatio; // top edge in px
        ...
        // Draw profit and loss panels (optional, comment out if not needed)
        drawLabelShort(
          ctx,
          profitCenterX,
          profitY,
          ...
          false,
          16, // offset for more space below profit box
        );
        drawLabelShort(
          ctx,
          lossCenterX,
          ...
          "rgba(60, 60, 60, 0.95)",
          true,
          16, // offset for more space above loss box
        );
        ...
        const statsY = paneView._entryLine.y * scope.verticalPixelRatio - 28; // 28px ABOVE entry line
        drawLabelShort(ctx, statsCenterX, statsY, statsLines, "#555", true);
        ctx.restore();
      }
    });
  }
